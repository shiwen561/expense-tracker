...
**个人记账助手**是一款轻量级的日常财务管理工具，帮助用户轻松记录每一笔收支。数据完全存储在浏览器本地（IndexedDB），无需注册账号、无需联网，隐私安全无忧。

### 适用场景

- 💸 **日常消费追踪** — 记录餐饮、交通、购物等日常开销，随时掌握消费去向
- 📊 **月度预算管理** — 设定月度预算，实时跟踪消费进度，避免超支
- 📈 **收支趋势分析** — 通过图表直观了解收入与支出的变化趋势
- 🤖 **智能记账** — 粘贴微信/支付宝的支付通知文本，AI 自动识别并填入账单信息
- 📲 **Android 自动记账** — 安装 Android 配套应用后，支付通知自动被抓取并生成账单，无需手动操作
- 📥 **历史账单导入** — 支持从微信、支付宝导出的账单文件（CSV / Excel）一键导入

### 设计理念

- **隐私优先** — 所有数据存储在本地浏览器，不上传任何服务器
- **极简操作** — 无需注册登录，打开即用；三种记账方式覆盖不同场景
- **智能辅助** — 集成 DeepSeek AI，让记账更省心（可选，无 API 也能正常使用）
- **多端覆盖** — Web 端随时随地打开，Android 端提供原生自动记账体验
